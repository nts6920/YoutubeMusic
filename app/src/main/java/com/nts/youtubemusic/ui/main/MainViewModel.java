...
import com.nts.youtubemusic.common.LiveEvent;
...
public class MainViewModel extends BaseViewModel {
    ...
    public MutableLiveData<String> commentLiveData = new MutableLiveData<>();
    public MutableLiveData<List<VideoChannel>> videoTrendingLiveDataVN = new MutableLiveData<>();
    public MutableLiveData<Comment> commentMutableLiveData = new MutableLiveData<>();
    public LiveEvent<MessageEvent> liveEventInternet = new LiveEvent<>();
    ...
    @Inject
    public MainViewModel(YoutubeRepository youtubeRepository, YoutubeAPI youtubeAPI) {
    ...
    public void searchVideo(String q, int maxResults) {
        List<VideoChannel> videoChannels = new ArrayList<>();

        youtubeRepository.getVideoSearch(Constant.API_KEY, Constant.PART, q, Constant.TYPE, maxResults).subscribe(new SingleObserver<Search>() {
            @Override
            public void onSubscribe(@NonNull Disposable d) {
                compositeDisposable.add(d);
            }
                        ...
                        .subscribe(new SingleObserver<Channel>() {
                            ...
                            public void onSuccess(@NonNull Channel channel) {
                                ...
                                youtubeAPI.getVideo(itemsList.get(0).getId().getVideoId(), Constant.API_KEY).flatMap(videoYoutube ->
                                        ...
                                        setVideo(videoChannels, videoYoutube, 7)).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<VideoYoutube>() {
                                    @Override
                                    public void onSubscribe(@NonNull Disposable d) {
                                        compositeDisposable.add(d);
                                    }
            ...
            public void onError(@NonNull Throwable e) {
                Timber.e(e);
                index = (index + 1) % listApi.size();
                liveEventInternet.postValue(new MessageEvent(Constant.CONNECT_SLOW_HOME));
                videoTrendingLiveDataVN.postValue(null);
